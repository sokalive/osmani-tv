 ...
 *   conflict?: Record<string, unknown>;
  ...
  constructor(userMessage, meta = {}) {
    ...
    this.provider = meta.provider ?? null;
    this.path = meta.path ?? null;
    this.title = meta.title ?? 'Taarifa';
    this.conflict = meta.conflict ?? null;
  }
}

/** THIS device already has an active subscription — fail closed (never key on phone). */
export class DeviceSubscriptionConflictError extends Error {
  /**
   * @param {string} userMessage
   * @param {{
   *   code?: string;
  ...
  constructor(userMessage, meta = {}) {
    super(userMessage);
    this.name = 'DeviceSubscriptionConflictError';
    this.userMessage = userMessage;
    this.code = meta.code ?? 'DEVICE_ALREADY_HAS_ACTIVE_SUBSCRIPTION';
    this.backendReason = meta.backendReason ?? null;
    this.httpStatus = meta.httpStatus ?? 409;
    this.provider = meta.provider ?? null;
    this.path = meta.path ?? null;
    this.title = meta.title ?? 'Kifurushi Kinaendelea';
    this.conflict = meta.conflict ?? null;
  }
}
 ...
 * @returns {boolean}
 */
export function isPaymentCreateOrderTimeout(errorLike) {
  return /payment-create-order:timeout/i.test(String(errorLike?.message ?? errorLike ?? ''));
}
...
export function formatCheckoutPaymentError(rawMessage, ctx = {}) {
  ...
  if (/payment-create-order:timeout/i.test(lower)) {
    return 'Malipo yanaendelea kwenye simu yako. Subiri kidogo ukimaliza PIN — usijaribu mara mbili.';
  }
