...
import { optimizeDisplayImageUrl, resolveMediaAssetUrl, resolveUploadImageCdnFallback, withImageCacheRevision } from './mediaDelivery';
...
export function normalizeBanner(raw, index) {
  ...
  const imageUrl = withImageCacheRevision(
    optimizeDisplayImageUrl(
      resolveMediaAssetUrl(String(raw?.image_url ?? raw?.imageUrl ?? '').trim()),
      { maxWidth: 840, quality: 82 },
    ),
    raw?.updated_at ?? raw?.updatedAt,
  );
  const imageFallbackUrl = resolveUploadImageCdnFallback(
    resolveMediaAssetUrl(String(raw?.image_url ?? raw?.imageUrl ?? '').trim()),
    { maxWidth: 840, quality: 82 },
  );
