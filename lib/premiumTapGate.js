...
import {
  explicitTapNeedsBoundedActiveGuard,
  mayShowPaymentAffordance,
} from './paymentAffordancePolicy';
...
export async function resolveExplicitPremiumTapSnapshot(getSnapshot, options = {}) {
  ...
  // Known inactive / payment-ready: open instantly — never wait on hydrate/guard.
  if (snapshotIsConfirmedInactive(snap) || snapshotIsReadyForPaymentFlow(snap)) {
    return snap;
  }
  ...
  snap = withCanonicalEntitlement(getSnapshot?.() ?? {});
  if (snapshotHasActiveSubscription(snap)) return snap;
  if (snapshotIsConfirmedInactive(snap) || snapshotIsReadyForPaymentFlow(snap)) return snap;
 ...
 * Delegates to mayShowPaymentAffordance so ACTIVE/STALE_ACTIVE never flash KULIPIA.
 * @param {object} input
 */
export function shouldShowKulipiaBadge(input) {
  return mayShowPaymentAffordance(input);
}
