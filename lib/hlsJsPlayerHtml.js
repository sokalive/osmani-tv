  ...
  <body>
    ...
    <script>
      (function () {
        ...
        var MAX_CONSECUTIVE_RECOVERIES = 8;
        ...
        function isLikelyLive() {
          ...
          if (!hls || !hls.levels || !hls.levels.length) return false;
          for (var i = 0; i < hls.levels.length; i++) {
            var d = hls.levels[i] && hls.levels[i].details;
            if (d && d.live) return true;
          }
          return false;
        }

        function buildRestorePoint() {
          var live = isLikelyLive();
          var t = null;
          if (live && hls && typeof hls.liveSyncPosition === 'number' && isFinite(hls.liveSyncPosition)) {
            t = hls.liveSyncPosition;
          } else if (video && isFinite(video.currentTime) && video.currentTime > 0) {
            t = video.currentTime;
          }
          return { live: live, time: t };
        }

        function applyPendingRestore() {
          if (!pendingRestore || !video) return;
          var target = pendingRestore.time;
          var live = pendingRestore.live;
          pendingRestore = null;
          if (typeof target !== 'number' || !isFinite(target) || target < 0) return;
          try {
            video.currentTime = live ? Math.max(0, target - 0.3) : target;
            post('recovery_attempt', {
              step: 'restore-position',
              reason: lastRecoveryReason || 'recovery',
              target: target,
              live: live,
            });
          } catch (e) {}
        ...
        function scheduleRecovery(reason) {
          ...
          var delay = Math.min(8000, 400 * Math.pow(2, Math.min(consecutiveRecoveries, 5)));
        ...
        function runRecoveryStep() {
          ...
          if (consecutiveRecoveries >= MAX_CONSECUTIVE_RECOVERIES) {
            ...
            recoveryStep = 0;
            post('recovery_failed', {
              reason: lastRecoveryReason || 'unknown',
              mode: 'max-consecutive',
          ...
          post('recovery_attempt', {
            ...
            reason: lastRecoveryReason || null,
            consecutive: consecutiveRecoveries,
            step_index: recoveryStep,
          });
          try {
            if (stepName === 'seek-nudge') {
              if (video.readyState >= 2 && !video.paused && isFinite(video.currentTime)) {
                video.currentTime = Math.max(0, video.currentTime + 0.1);
              }
            } else if (stepName === 'startLoad') {
              if (hls && typeof hls.startLoad === 'function') hls.startLoad();
            } else if (stepName === 'recoverMediaError') {
              if (hls && typeof hls.recoverMediaError === 'function') hls.recoverMediaError();
            } else {
              pendingRestore = buildRestorePoint();
              recreateHls();
            }
            consecutiveRecoveries += 1;
            recoveryStep = Math.min(recoveryStep + 1, 3);
          } catch (e) {
            post('recovery_failed', {
              reason: lastRecoveryReason || null,
              step: stepName,
        ...
        function createHlsInstance() {
          ...
          hls.on(Hls.Events.ERROR, function (event, data) {
            ...
            scheduleRecovery('fatal-' + String(data.type || 'unknown'));
          });
        ...
        video.addEventListener('playing', function () {
          stallReported = false;
          lastObservedWall = Date.now();
          post('video_playing', null);
          if (consecutiveRecoveries > 0 || recoveryStep > 0) {
        ...
        setInterval(function () {
          ...
          if (Math.abs(video.currentTime - lastTime) <= 0.02) {
            if (Date.now() - lastObservedWall > STALL_MS) {
              stallReported = true;
              post('stall_detected', {
                lastTime: lastTime,
              ...
              scheduleRecovery('stall_watchdog');
            }
          } else {
            lastTime = video.currentTime;
            lastObservedWall = Date.now();
        ...
        window.addEventListener('online', function () {
          post('recovery_attempt', { step: 'network-online', reason: 'online-event' });
          scheduleRecovery('network-online');
        });
        ...
        setInterval(function () {
          ...
          // If the browser reports a stalled network state, schedule conservative recovery.
          if (video.networkState === 2 || video.networkState === 3) {
            scheduleRecovery('network-state-' + String(video.networkState));
          }
        }, 4000);

        setInterval(function () {
          if (Date.now() - lastTimeAt > STALL_MS * 3 && !video.paused && !video.ended && video.readyState >= 2) {
            stallReported = true;
            post('stall_detected', {
              lastTime: lastTime,
              currentTime: video.currentTime,
