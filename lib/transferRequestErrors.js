...
export function formatTransferRequestUserMessage(errorLike, httpStatus = 0) {
  ...
  if (lower.includes('cooldown_active') || lower.includes('cooldown')) {
    return 'Subiri kidogo kabla ya kuomba uhamisho mwingine.';
  }
  if (lower.includes('transfer_daily_limit') || lower.includes('daily limit')) {
    return 'Umevuka kikomo cha uhamisho wa leo. Jaribu kesho.';
  }
  if (lower.includes('transfer_weekly_limit') || lower.includes('weekly limit')) {
    return 'Umevuka kikomo cha uhamisho wa wiki. Jaribu baadaye.';
  }
