...
import { shouldReloadAfterOtaFetch, isStalePlaybackBundle, hasKifurushiKimekwishaGateRemoved } from './otaBootGatePolicy';
...
export async function syncExpoUpdateBundle(reason = 'manual', opts = {}) {
  ...
  try {
    ...
    if (
      reloadIfNew &&
      fetch.isNew === true &&
      shouldReloadAfterOtaFetch(staleAtSessionStart)
    ) {
...
function maybeForegroundSync() {
  ...
  const stalePopupBundle = !hasKifurushiKimekwishaGateRemoved();
  void syncExpoUpdateBundle('foreground', {
    reloadIfNew: stalePopupBundle,
