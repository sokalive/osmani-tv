 ...
 * - Cold start + in-session checks. Reload immediately when safe (not in ChannelPlayer).
 * - While watching a channel: download/stage only; reload when leaving playback.
...
import {
  isChannelPlaybackActive,
  subscribeChannelPlaybackActive,
} from './otaSessionGate';
...
/** While app stays open on a current bundle, still hunt new OTAs in-session. */
const IN_SESSION_RECHECK_MS = 90 * 1000;
...
let inSessionTimer = null;
let sessionHuntStartedAt = 0;
let playbackUnsub = null;
/** Downloaded while ChannelPlayer was active — apply on leave. */
let pendingReloadAfterPlayback = false;
...
export async function getExpoUpdatesDiagnostics() {
  ...
  try {
    return {
      ...
      pendingReloadAfterPlayback,
      channelPlaybackActive: isChannelPlaybackActive(),
      popupMarkers: {
...
async function triggerReload(reason, extras = {}) {
  if (reloadTriggered) {
    return { skipped: true, reason: 'reload_already_triggered' };
  }
  reloadTriggered = true;
  pendingReloadAfterPlayback = false;
  ...
  logProd('app_reload_automatic', { reason, ...extras });
  await Updates.reloadAsync();
  return { reloaded: true, reason };
}

/**
 * Apply a staged OTA after leaving ChannelPlayer (or when already safe).
 * @param {string} [reason]
 */
export async function applyPendingExpoUpdateIfAny(reason = 'playback-exit') {
  if (!isExpoUpdatesRuntimeEnabled()) return { skipped: true, reason: 'disabled' };
  if (!pendingReloadAfterPlayback) return { skipped: true, reason: 'none_pending' };
  if (isChannelPlaybackActive()) {
    logProd('ota_reload_still_deferred', { reason, pending: true });
    return { deferred: true, reason: 'still_in_playback' };
  }
  logProd('ota_pending_apply', { reason });
  return triggerReload(reason, { pendingApply: true });
}

/**
 * Check EAS for a compatible update (same runtimeVersion) and download if available.
 * Reloads immediately when not in channel playback; otherwise stages for later.
 *
 * @param {string} [reason]
 * @param {{
 *   reloadIfNew?: boolean,
...
export async function syncExpoUpdateBundle(reason = 'manual', opts = {}) {
  ...
  try {
    ...
    if (reloadIfNew === true && fetch.isNew === true) {
      if (isChannelPlaybackActive()) {
        pendingReloadAfterPlayback = true;
        out.appliesOnNextLaunch = false;
        out.deferredUntilPlaybackExit = true;
        lastResult = { at: Date.now(), reason, ...out };
        logProd('ota_reload_deferred_playback', out);
        return out;
      }
      out.appliesOnNextLaunch = false;
      ...
      await triggerReload(reason, { staleAtSessionStart });
      return out;
    }
...
function maybeForegroundSync() {
  ...
  if (Date.now() - lastCheckAt < throttleMs) {
    if (pendingReloadAfterPlayback && !isChannelPlaybackActive()) {
      void applyPendingExpoUpdateIfAny('foreground-pending');
    }
    return;
  }
...
function scheduleInSessionSync() {
  if (inSessionTimer) {
  ...
  inSessionTimer = setTimeout(() => {
    ...
    if (AppState.currentState === 'active') {
      if (pendingReloadAfterPlayback && !isChannelPlaybackActive()) {
        void applyPendingExpoUpdateIfAny('in-session-pending');
      } else if (Date.now() - lastCheckAt >= IN_SESSION_RECHECK_MS) {
        void syncExpoUpdateBundle('in-session', { reloadIfNew: true });
      }
    }
    scheduleInSessionSync();
  }, IN_SESSION_RECHECK_MS);
}
...
/**
 * Start background EAS Update sync (startup + in-session + throttled resume).
 * @returns {() => void}
 */
export function startExpoUpdatesClient() {
  ...
  if (!playbackUnsub) {
    playbackUnsub = subscribeChannelPlaybackActive((active) => {
      if (!active) {
        void applyPendingExpoUpdateIfAny('playback-exit');
      }
    });
  }
  scheduleLongIntervalSync();
