 ...
 * @returns {string}
 */
function readChannelAuthorizedPackageName(channel) {
  if (!channel || typeof channel !== 'object') return '';
  const raw =
    channel.authorizedPackageName ??
    channel.authorized_package_name ??
    channel.authorizedPackage ??
    channel.authorized_package ??
    '';
...
export function buildStreamRequestHeaders(channel) {
  ...
  const withPackage = (() => {
    const pkg = readChannelAuthorizedPackageName(channel);
    if (!pkg) return headers;
    return {
      ...headers,
      'X-Package-Name': pkg,
      'Authorized-Package-Name': pkg,
    };
  })();
  return withPackage && Object.keys(withPackage).length ? withPackage : undefined;
}

/**
 * @param {unknown} pt
 * @returns {'exo' | 'webview' | 'vlc' | 'native' | 'ijk' | 'chrome'}
 */
