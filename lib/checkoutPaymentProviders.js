...
/** Admin API may expose Aurax as `auraxpay`, `aurax`, or `aurax_pay`. */
function parseAuraxpayEnabled(body) {
  if (!body || typeof body !== 'object') return false;
  if (body.auraxpay === true) return true;
  if (body.aurax === true) return true;
  if (body.aurax_pay === true || body.auraxPay === true) return true;
  return false;
}
...
export function parseCheckoutProvidersResponse(body) {
  const flags = {
    ...
    auraxpay: parseAuraxpayEnabled(body),
  ...
  return {
    ...
    logos: {
      ...
      auraxpay: pickLogo(body, 'auraxpay') ?? pickLogo(body, 'aurax'),
    },
  };
}
