...
export function enrichCanonicalSubscriptionTiming(input = {}) {
  ...
  // Prefer assigned Admin plan duration over stacked startedAt→expiresAt span.
  const planDays = pickNumber(input.planDurationDays, input.plan_duration_days);
  if (planDays != null && planDays > 0) {
    displayDurationDays = Math.trunc(planDays);
  } else if (startedMs != null && expiresMs != null && expiresMs > startedMs) {
...
export function resolveDisplayDurationDays(details) {
  ...
  // Prefer package metadata over startedAt→expiresAt span (span can exceed package).
