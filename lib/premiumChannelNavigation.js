...
import { PREMIUM_GATE_MAX_MS, snapshotHasActiveSubscription } from './premiumTapGate';
 ...
 * @param {{ playerChannel: object; cardIsPremium: boolean; navigation: object; openPaymentModal: () => void | Promise<void>; verifySubscriptionBeforePlay?: () => Promise<boolean>; verifySubscriptionInBackground?: (reason?: string) => void; security: object; Alert: { alert: (title: string, msg: string) => void } }} ctx
 */
export async function openPremiumChannelFromSnapshot(snapshot, ctx) {
  ...
  if (snapshotHasActiveSubscription(snapshot)) {
    if (verifySubscriptionBeforePlay) {
      const ok = await verifySubscriptionBeforePlay('channel-tap-premium');
      if (!ok) {
        logChannelCardTap('navigation_blocked', { channelKey, reason: 'revoked_or_unsubscribed' });
        void openPaymentModal();
        return;
      }
    }
