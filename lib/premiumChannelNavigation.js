...
export async function openPremiumChannelFromSnapshot(snapshot, ctx) {
  ...
  if (isInstructionVideoChannel(playerChannel)) {
    logChannelCardTap('navigation_navigate', { channelKey, path: 'instruction_video' });
  ...
  if (!snapshot.isSubscribed) {
    const trialEligible = shouldApplyTrialWatch({
      isSubscribed: false,
      freeMode: snapshot.freeMode,
      trialWatchSettings: snapshot.trialWatchSettings,
    });
    if (trialEligible) {
  ...
  void reportUserCenterEvent('premium_unlock', {
    channel_id: channelKey,
    channel_name: String(playerChannel?.name ?? playerChannel?.title ?? '').trim(),
  });
  navigation.navigate('ChannelPlayer', {
    channel: playerChannel,
    trialWatchBootstrap: null,
