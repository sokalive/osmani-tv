...
export async function openPremiumChannelFromSnapshot(snapshot, ctx) {
  ...
  if (premiumContent) {
    ...
    if (snapshotIsResolving(snap) && awaitEntitlementForTap) {
    ...
    if (!snapshotIsConfirmedInactive(snap) && !mayOpenPaymentPopup(snap.entitlementPhase)) {
    ...
    if (mayOpenPaymentPopup(snap.entitlementPhase) || snapshotIsConfirmedInactive(snap)) {
    ...
    logChannelCardTap('navigation_deferred', {
      channelKey,
      reason: 'entitlement_ambiguous_no_popup',
      phase: snap.entitlementPhase ?? null,
    });
    onEntitlementDeferred?.(playerChannel);
    return 'deferred';
  ...
  logChannelCardTap('navigation_deferred', { channelKey, reason: 'payment_modal_unknown' });
  return 'deferred';
}
