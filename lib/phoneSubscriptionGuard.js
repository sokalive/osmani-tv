...
export const PHONE_GUARD_FALLBACK_TITLE = 'Namba Hii Tayari Inatumika';
...
export function buildPhoneSubscriptionGuardFallbackMessage(existingPackage, remainingDays) {
  const pkg = String(existingPackage ?? '').trim();
  const days = Number(remainingDays);
  const hasPkg = pkg.length > 0;
  const hasDays = Number.isFinite(days) && days > 0;

  if (hasPkg && hasDays) {
    return (
      `Namba hii tayari ina kifurushi cha ${pkg} kinachoendelea kwenye kifaa kingine.\n\n` +
      `Kifurushi kitaisha baada ya takribani ${days} siku.\n\n` +
      'Tafadhali tumia namba nyingine kulipia au subiri kifurushi hiki kiishe ndipo ulipe kwenye kifaa hiki.'
    );
  }

  return (
    'Namba hii tayari ina kifurushi kinachoendelea kwenye kifaa kingine.\n\n' +
    'Tafadhali tumia namba nyingine kulipia au subiri kifurushi cha sasa kiishe ndipo ulipe kwenye kifaa hiki.'
  );
}
