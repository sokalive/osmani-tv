...
import { ensureOneSignalPushRegistration } from './oneSignalPushRegistration';
...
/** Always logged — used to verify cold-start tap + OneSignal click attribution in logcat. */
function logNotificationClick(tag, payload) {
  try {
    console.log('[OneSignal]', tag, payload);
  } catch {
    /* ignore */
...
function extractOpenUrlFromClickEvent(event) {
  ...
  if (data && typeof data === 'object' && !Array.isArray(data)) {
    ...
    const channelId = d.channel_id ?? d.channelId ?? d.target_id ?? d.targetId;
    if (channelId != null && String(channelId).trim()) {
      return `osmani://channel/${encodeURIComponent(String(channelId).trim())}`;
    }
  }
  return null;
}
...
function dispatchNotificationClick(event) {
  try {
    ...
    logNotificationClick('notification.click', {
      notificationId,
    ...
    if (!url) {
      logNotificationClick('notification.click.no-url', {
    ...
    logNotificationClick('notification.click.error', { error: String(e) });
  }
}

/** Shown while app is in foreground; text must display even if rich image fails on old native SDK. */
function handleForegroundWillDisplay(event) {
  let notification = null;
  try {
    notification = event.getNotification();
    applyAudienceSnapshotFromNotification(notification);
  } catch (e) {
    logNotificationClick('foregroundWillDisplay.parse-error', { error: String(e) });
  }
  try {
    (notification ?? event.getNotification()).display();
  } catch (e) {
    logNotificationClick('foregroundWillDisplay.display-error', { error: String(e) });
    try {
      event.getNotification()?.display?.();
    } catch (e2) {
      logNotificationClick('foregroundWillDisplay.display-fallback-error', { error: String(e2) });
    }
  }
}

let pushRegistrationRepairTimer = null;
...
const onPushSubscriptionChange = (event) => {
  ...
  if (pushRegistrationRepairTimer) clearTimeout(pushRegistrationRepairTimer);
  pushRegistrationRepairTimer = setTimeout(() => {
    void ensureOneSignalPushRegistration('pushSubscription.change');
  }, 400);
};
...
export function bootstrapOneSignalNative() {
  ...
  logNotificationClick('bootstrap.listeners', {
    click: true,
    foregroundWillDisplay: true,
    note: 'Listeners are module-scoped; setupOneSignal remount only updates onOpenUrl handler.',
  });
