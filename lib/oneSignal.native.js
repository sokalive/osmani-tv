...
/**
 * Foreground: parse audience only — OneSignal v5 auto-displays rich media when we do
 * not call preventDefault() or manual display(). Manual display() strips big_picture.
 */
function handleForegroundWillDisplay(event) {
  try {
    applyAudienceSnapshotFromNotification(event.getNotification());
  } catch (e) {
