  ...
  for (let attempt = 1; attempt <= EMBEDDED_LAUNCH_MAX_ATTEMPTS; attempt += 1) {
    try {
      ...
      if (lastResult.ok && lastResult.available === true && lastResult.isNew === true) {
        ...
        // Always reload when boot gate fetched a new bundle — do not depend on
        // isEmbeddedLaunch (unreliable). This is what left VPS users on the
        // expired-package popup after prior OTAs.
        setEmbeddedOtaPhase('reloading');
