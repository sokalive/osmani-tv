...
export const EMBEDDED_LAUNCH_OTA_RETRIES = 3;
  ...
  gatePromise = (async () => {
    ...
    for (let attempt = 1; attempt <= EMBEDDED_LAUNCH_OTA_RETRIES; attempt += 1) {
      try {
        ...
        if (lastResult.ok && lastResult.available === true && !lastResult.embeddedLaunchReload) {
          logBoot('force_reload_missing', lastResult);
          try {
            await Updates.reloadAsync();
            return { ...lastResult, embeddedLaunchReload: true, forcedGateReload: true };
          } catch (e) {
            logBoot('force_reload_failed', e?.message ?? e);
          }
        }
