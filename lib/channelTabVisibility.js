...
/**
 * Sort home “Zote” grid: popular first, then featured, then stable name.
 * @param {Record<string, unknown>} a
 * @param {Record<string, unknown>} b
 */
export function compareHomeMixChannels(a, b) {
  const ap = channelIsPopular(a) ? 1 : 0;
  const bp = channelIsPopular(b) ? 1 : 0;
  if (ap !== bp) return bp - ap;
  const af = channelIsFeatured(a) ? 1 : 0;
  const bf = channelIsFeatured(b) ? 1 : 0;
