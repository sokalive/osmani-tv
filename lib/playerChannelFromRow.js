import { resolveMediaAssetUrl, isStreamProxyUrl } from './mediaDelivery';
...
export function buildPlayerChannelFromRow(raw, index, freeMode) {
  ...
  const primaryCandidate =
    playbackUrl ||
    (isStreamProxyUrl(streamProxyPrimary) ? streamProxyPrimary : '') ||
    rawUrl;
  const primaryUrl = resolveMediaAssetUrl(primaryCandidate);
  ...
  const backup2Raw =
    typeof raw?.backupStream2 === 'string' && raw.backupStream2.trim()
      ...
      : typeof raw?.backup_stream_2 === 'string' && raw.backup_stream_2.trim()
        ...
        : '';
  const backup1 = backup1Raw ? resolveMediaAssetUrl(backup1Raw) : '';
  const backup2 = backup2Raw ? resolveMediaAssetUrl(backup2Raw) : '';
