...
import { sanitizePlaybackUrl } from './freshInstallPlaybackRecovery';
import {
...
export function buildPlayerChannelFromRow(raw, index, freeMode) {
  ...
  const primaryUrl = sanitizePlaybackUrl(playbackPlan.playUrl);
  const sanitizedDirectUrl = sanitizePlaybackUrl(playbackPlan.directUrl);
  const sanitizedProxyFallback = sanitizePlaybackUrl(playbackPlan.proxyFallbackUrl);
  ...
  return {
    ...
    directStreamUrl: sanitizedDirectUrl,
    direct_stream_url: sanitizedDirectUrl,
    proxyFallbackUrl: sanitizedProxyFallback,
    proxy_fallback_url: sanitizedProxyFallback,
