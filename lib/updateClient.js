...
import { readNativeAndroidVersionCode, PLAY_OTA_MIN_VERSION_CODE } from './playVpsApiHost';
...
/** v16+ soft dismiss until next cold launch; v15 re-shows every resume. */
...
function reassertSoftOverlayVisibility() {
  ...
  // v15 cannot OTA-migrate — always re-show update UI on resume when outdated.
  if (installed !== PLAY_OTA_MIN_VERSION_CODE - 1) return;
...
function applyUpdateInfo(info, { reason = 'unknown' } = {}) {
  ...
  if (installed >= PUBLISHED_PLAY_VERSION_CODE) {
    ...
    installed >= PLAY_OTA_MIN_VERSION_CODE &&
    softUpdateDismissed &&
    reason !== 'app-launch'
  ) {
    visible = false;
...
export function dismissSoft() {
  ...
  const installed = readInstalledVersionCode();
  if (installed >= PLAY_OTA_MIN_VERSION_CODE && installed < PUBLISHED_PLAY_VERSION_CODE) {
