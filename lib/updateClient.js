...
function reassertSoftOverlayVisibility() {
  const installed = readInstalledVersionCode();
  // v15–v23: re-show update UI on every resume until Play v24+.
  if (installed <= 0 || installed >= PUBLISHED_PLAY_VERSION_CODE) return;
  softUpdateDismissed = false;
  if (!lastUpdateInfo) return;
  const gated = normalizeUpdateInfo(lastUpdateInfo);
  const decision = gated?.decision ?? 'NONE';
  const latest = latestVersionCodeTarget(gated);
  if (
    installed > 0 &&
    installed < PUBLISHED_PLAY_VERSION_CODE &&
    shouldShowFor(decision) &&
...
/** Auto-download session — overlay stays modal until APK bytes are on disk. */
function isAutoDownloadInProgress() {
  if (lastUpdateInfo?.autoDownload !== true) return false;
  const decision = lastUiState.decision ?? lastUpdateInfo?.decision ?? 'NONE';
  if (!shouldShowFor(decision)) return false;
  if (lastUiState.downloaded) return false;
  if (lastUiState.installing || lastUiState.needsUnknownSourcesPermission) return false;
  return true;
}
...
function applyUpdateInfo(info, { reason = 'unknown' } = {}) {
  ...
  if (installed >= PUBLISHED_PLAY_VERSION_CODE) {
    visible = false;
  } else if (softUpdateDismissed && reason !== 'app-launch' && reason !== 'app-resume') {
    visible = false;
  }
  if (
    gated?.autoDownload === true &&
    shouldShowFor(decision) &&
    outdated &&
    !lastUiState.downloaded &&
    !lastUiState.installing &&
    !lastUiState.needsUnknownSourcesPermission
  ) {
    visible = true;
  }
  const tag =
...
export function cancelDownload() {
  ...
  if (isAutoDownloadInProgress()) return;
