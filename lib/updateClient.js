...
/** Server target from update-check (latest_version_code, else min_supported). */
function serverVersionCodeTarget(info) {
  const latest = Number(info?.latestVersionCode ?? info?.latest_version_code ?? 0);
  const min = Number(info?.minSupportedVersionCode ?? info?.min_supported_version_code ?? 0);
...
/**
 * Backend may return FORCE/SOFT globally; suppress UI when this install already satisfies
 * the published version_code threshold (installed >= server target).
 */
function normalizeUpdateInfo(info) {
  if (!info || typeof info !== 'object') return info;
  const installed = Number(info.installedVersionCode ?? info.installed_version_code ?? 0);
  const server = serverVersionCodeTarget(info);
  ...
  if (
    ...
    log('[NONE]', 'update suppressed — installed >= server version', {
      installed,
      server,
      rawDecision: raw,
    });
    return {
      ...
      rawDecision: raw,
      serverVersionCodeTarget: server,
    };
  }
  return info;
}
...
async function performCheckOnce(reason) {
  ...
  try {
    ...
    warn('[CHECK_RESP]', 'failed', lastCheckError);
    notifyDebug();
    return null;
  }
}
...
export function getDebugSnapshot() {
  return {
    ...
    derived: {
      ...
      serverVersionCodeTarget: serverVersionCodeTarget(lastUpdateInfo),
