...
/** Auto-download APK fetch in progress — overlay must stay modal until complete. */
function isAutoDownloadInProgress() {
  if (lastUpdateInfo?.autoDownload !== true) return false;
  return lastUiState.downloading === true || lastUiState.verifying === true;
}
...
function recomputeUiFromNative(nativeState) {
  ...
  setUi(next);
  if (
    lastUpdateInfo?.autoDownload === true &&
    (next.downloading === true || next.verifying === true)
  ) {
    setUi({ visible: true });
  }
}
...
function applyUpdateInfo(info, { reason = 'unknown' } = {}) {
  ...
  if (
    ...
    (lastUiState.downloading || lastUiState.verifying)
  ) {
    visible = true;
  }
  const tag =
