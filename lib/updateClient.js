...
import { isApkInstallerEnabled } from './apkInstallerConfig';
import {
  applyVersionGate,
  isOutdated,
  mergeUpdateInfo,
...
function isAndroidNativeAvailable() {
  return isApkInstallerEnabled() && Platform.OS === 'android' && OsmaniUpdate != null;
}
...
export function startUpdateClient() {
  ...
  if (!isAndroidNativeAvailable()) {
    log('[OTA_INIT]', 'skipped — installer disabled, not android, or native module unavailable', {
      apkInstallerEnabled: isApkInstallerEnabled(),
    });
    notifyDebug();
    return;
  }
