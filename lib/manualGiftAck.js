...
/** True when acknowledge-manual-gift reports no server-side pending grant. */
export function isNoPendingManualGiftError(errorLike) {
  const msg = String(errorLike?.message ?? errorLike ?? '').toLowerCase();
  return msg.includes('no pending manual gift matched');
}

/** Drop stale pending popup key (e.g. after OTA or failed ack). */
export async function purgeStaleManualGiftPendingKey() {
  await clearPendingManualGiftKey();
}

/** True when local storage already recorded acknowledgement for this gift key. */
export async function isManualGiftKeyAcknowledged(key) {
...
/**
 * Persist ack + drop pending after successful acknowledge-manual-gift API.
 * Popup must not reopen for this key unless backend issues a new grant.
 */
export async function finalizeManualGiftAcknowledgement(key) {
  const k = String(key ?? '').trim();
  if (!k) return;
  await writeManualGiftAck(k);
  await clearPendingManualGiftKey();
}
