...
export function computePollIntervalMs({ elapsedMs, waitingState, retryable, paymentConfirmed }) {
  ...
  // After provider confirm: stay aggressive for 2 minutes so ACTIVE is caught immediately.
  if (waitingState === APP_WAITING_STATE.PROVIDER_CONFIRMED_ACTIVATING || paymentConfirmed) {
    if (elapsed < 60_000) return 500 + jitter();
    if (elapsed < 120_000) return 800 + jitter();
    return 1500 + jitter();
  }
