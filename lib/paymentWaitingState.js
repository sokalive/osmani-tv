 ...
 * PENDING/RETRYING may advance to FAILED (real cancel).
 * FAILED must never overwrite PROVIDER_CONFIRMED_ACTIVATING / ACTIVE (stale poll).
...
export function shouldAcceptWaitingStateUpdate(current, incoming) {
  ...
  // Stale FAILED after money/activation progress must not kill Hongera path.
  if (
    inc === APP_WAITING_STATE.FAILED &&
    (cur === APP_WAITING_STATE.PROVIDER_CONFIRMED_ACTIVATING ||
      cur === APP_WAITING_STATE.ACTIVE)
  ) {
    return false;
  }
  // Explicit PENDING/RETRYING → FAILED (rank alone would reject: FAILED=-20 < PENDING=0).
  if (inc === APP_WAITING_STATE.FAILED) {
    return (
      cur === APP_WAITING_STATE.PAYMENT_PENDING ||
      cur === APP_WAITING_STATE.RETRYING ||
      cur === APP_WAITING_STATE.FAILED
    );
  }
...
/**
 * SonicPesa can briefly report FAILED with no provider_order_id while STK is still
 * initiating (`provider_initiation: pending`). That must not abort the waiting UI.
 *
 * @param {{
 *   status?: string;
 *   appWaitingState?: string;
 ...
 * @returns {boolean}
 */
export function isPrematureFailedPaymentStatus(result, elapsedMs = 0) {
  if (!result || typeof result !== 'object') return false;
  const waiting = String(result.appWaitingState ?? '');
  const status = String(result.status ?? '').toUpperCase();
  if (waiting !== APP_WAITING_STATE.FAILED && status !== 'FAILED') return false;
  ...
  const providerOrderId =
    ...
    raw.provider_txn_id ??
    null;
  const initiation = String(
    raw.provider_initiation ?? raw.providerInitiation ?? '',
  )
    .trim()
    .toLowerCase();
  const elapsed = Math.max(0, Number(elapsedMs) || 0);

  // No provider txn yet — STK not definitively started / reconciled.
  if (providerOrderId == null || String(providerOrderId).trim() === '') {
    if (initiation === 'pending' || initiation === '' || initiation === 'none') {
