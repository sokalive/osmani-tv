...
export function computePollIntervalMs({ elapsedMs, waitingState, retryable, paymentConfirmed }) {
  ...
  // After provider confirm: stay aggressive so ACTIVE / Hongera is caught immediately.
  if (waitingState === APP_WAITING_STATE.PROVIDER_CONFIRMED_ACTIVATING || paymentConfirmed) {
    if (elapsed < 60_000) return 250 + Math.floor(Math.random() * 80);
    if (elapsed < 120_000) return 450 + Math.floor(Math.random() * 100);
    return 900 + Math.floor(Math.random() * 120);
  }
  ...
  if (elapsed < 5000) return 600 + jitter();
  if (elapsed < 60_000) return 1500 + jitter();
  return 2200 + jitter();
}
