 ...
 * Legacy Render / IP stream URLs are rewritten to the active API host or BunnyCDN.
 *
 * Upload images: VPS serves new files at `${resolveApiBaseUrl()}/uploads/...`. Legacy copies may
 * exist only on BunnyCDN — keep VPS as primary and use {@link resolveUploadImageCdnFallback} on error.
 */

import { isVpsApiTarget, resolveApiBaseUrl } from './apiBaseUrl';
...
/** @param {string} host */
function isCurrentApiUploadHost(host) {
  return String(host ?? '').toLowerCase() === adminApiHost();
}
...
export function rewriteLegacyRenderMediaUrl(input) {
  ...
  if (s.startsWith('/')) {
    ...
    if (s.startsWith('/uploads/')) {
      return `${isVpsApiTarget() ? resolveApiBaseUrl() : resolveMediaCdnBase()}${s}`;
    }
  ...
  try {
    ...
    // New admin uploads are stored on VPS disk — do not rewrite to Bunny (stale pull origin).
    if (isAdminUpload && isCurrentApiUploadHost(host)) {
      return s;
    }
...
/**
 * Same upload path on BunnyCDN — for legacy files not yet on VPS disk.
 * @param {unknown} input
 * @returns {string}
 */
export function toCdnEquivalentUploadUrl(input) {
  const s = String(input ?? '').trim();
  if (!s) return '';
  try {
    const u = new URL(s.startsWith('/') ? `${resolveApiBaseUrl()}${s}` : s);
...
/**
 * Bunny fallback when a VPS `/uploads/` URL 404s (historical CDN-only assets).
 * @param {unknown} primary
 * @param {{ maxWidth?: number; quality?: number }} [opts]
 * @returns {string|null}
 */
export function resolveUploadImageCdnFallback(primary, opts = {}) {
  const cdn = toCdnEquivalentUploadUrl(primary);
  if (!cdn) return null;
  const primaryNorm = String(primary ?? '').trim();
  if (!primaryNorm || cdn === primaryNorm) return null;
  return optimizeDisplayImageUrl(cdn, opts);
}

/**
 * Stable cache identity when admin replaces an image (uses DB updatedAt, not render time).
 * @param {unknown} url
 * @param {unknown} revision updated_at / updatedAt / version
 * @returns {string}
 */
export function withImageCacheRevision(url, revision) {
