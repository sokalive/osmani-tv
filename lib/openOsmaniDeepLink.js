...
import { awaitPremiumSnapshotCapped, verifySubscriptionInBackground } from './premiumTapGate';
import { grantPremiumAccessIntent, hasFreshPremiumAccessIntent, consumePremiumAccessIntent, clearPremiumAccessIntent } from './premiumAccessIntent';
import { mayOpenPremiumModalFromExplicitTap } from './premiumAccessPromptPolicy';
...
export async function openOsmaniDeepLink(url, ctx) {
  ...
  const snapshot =
    isPremium && !ctx.freeMode
      ? await awaitPremiumSnapshotCapped(
  ...
  await openPremiumChannelFromSnapshot(snapshot, {
    ...
    openPaymentModal: () => {
      ...
      if (!hasFreshPremiumAccessIntent()) return;
