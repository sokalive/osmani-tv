...
export async function openOsmaniDeepLink(url, ctx) {
  ...
  if (isPremium && !ctx.freeMode && !ctx.premiumPlaybackReady) {
    const quick = ctx.getPremiumAccessSnapshot?.() ?? null;
    const unpaidPremiumTap = quick && !quick.isSubscribed;
    if (!unpaidPremiumTap) {
  ...
  const quickSnapshot = ctx.getPremiumAccessSnapshot?.();
  const unpaidPremium =
    isPremium && !ctx.freeMode && quickSnapshot && !quickSnapshot.isSubscribed;
  const snapshot = unpaidPremium
    ? quickSnapshot
    : await ctx.awaitPremiumAccessSnapshot();
