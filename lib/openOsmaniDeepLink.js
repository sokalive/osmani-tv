...
import { resolveChannelTapAccessSnapshot } from './premiumChannelTapSnapshot';
  ...
  const isActive =
    raw?.isActive !== undefined
      ...
      : raw?.active !== undefined
        ? Boolean(raw.active)
        : true;
  return showInApp && isActive && instructionVideoVisibleForInstall(raw, readNativeAndroidVersionCode());
}
 ...
 *   awaitRecoverBoot?: () => Promise<boolean>;
 *   getPremiumAccessSnapshot?: () => PremiumAccessSnapshot;
...
export async function openOsmaniDeepLink(url, ctx) {
  ...
  const isFree = ctx.freeMode || !isPremium;

  if (isPremium && !ctx.freeMode) {
    const snapshotSync =
      ctx.getPremiumAccessSnapshot?.() ?? {
        premiumPlaybackReady: true,
        isSubscribed: Boolean(ctx.isSubscribed),
        freeMode: ctx.freeMode,
        trialWatchSettings: null,
      };
    if (snapshotSync.isSubscribed !== true) {
      await openPremiumChannelFromSnapshot(snapshotSync, {
        playerChannel,
        ...
        Alert,
        requireUpdateBeforeChannelPlayback: ctx.requireUpdateBeforeChannelPlayback,
        onChannelUpdateRequired: ctx.onChannelUpdateRequired,
      });
      return { ok: true };
    }
  }

  if (isPremium && !ctx.freeMode && !ctx.isSubscribed) {
    await ctx.awaitRecoverBoot?.();
  }

  console.log('[deep-link] channel', { channelId, name: playerChannel?.name ?? null });
  const { snapshot } = await resolveChannelTapAccessSnapshot({
    ...
    awaitPremiumAccessSnapshot:
      ctx.awaitPremiumAccessSnapshot ??
      (async () =>
        ctx.getPremiumAccessSnapshot?.() ?? {
          premiumPlaybackReady: true,
          isSubscribed: Boolean(ctx.isSubscribed),
    ...
    premiumPlaybackReady: ctx.premiumPlaybackReady ?? true,
