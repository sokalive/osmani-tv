...
/**
 * Account Expiry box: date only in Africa/Dar_es_Salaam, `dd/mm/yyyy`.
 * Example: `29/07/2026`. No time, no long formatted text.
 */
export function formatSubscriptionExpiryDate(iso) {
  if (!iso) return '—';
  const p = toDarEsSalaamParts(iso);
  if (!p) return String(iso);
