...
/** When true, verify skips fast status probe so revoke/grant reconciles against VPS verify.
 * Transport failures still must NOT clear a known-active UI — only authoritative inactive does.
 */
export function isAuthoritativeReconcileReason(reason) {
  const r = String(reason ?? '');
  if (!r) return false;
  // SSE reconnect is not entitlement evidence — never treat as authoritative loss path.
  if (r === 'sse:sync_stream_connected' || r.includes('__sync_stream_connected')) return false;
  return (
