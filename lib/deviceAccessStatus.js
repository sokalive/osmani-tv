...
export function resolveDeviceAccessStatus(input = {}) {
  ...
  const enforcement = resolveEnforcement({
    ...
    intelAccessOpen: serverIntelOpen,
  });
  ...
  return {
    ...
    enforcementReason: enforcement.enforcementReason ?? null,
    enforcementTrigger: enforcement.enforcementTrigger ?? null,
    diagnostics: {
      serverIntelBlocked,
      serverIntelOpen,
      serverIntelStatus,
...
export function logDeviceAccessVerification(status, tag = 'verify') {
  const line = {
    ...
    enforcementReason: status.enforcementReason ?? status.diagnostics?.enforcementReason ?? null,
    enforcementTrigger: status.enforcementTrigger ?? status.diagnostics?.enforcementTrigger ?? null,
    ...status.diagnostics,
  };
