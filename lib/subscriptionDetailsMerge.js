...
export function mergeSubscriptionDetails(prev, incoming) {
  ...
  const prevPlanId = String(prev.planId ?? prev.plan_id ?? '').trim();
  const incomingPlanId = String(incoming.planId ?? incoming.plan_id ?? '').trim();
  const planIdentityChanged =
    prevPlanId !== '' &&
    incomingPlanId !== '' &&
    prevPlanId !== incomingPlanId;
  ...
  const amountChanged =
    ...
    prevAmount !== incomingAmount;

  // Backend assigned a different package — replace plan fields; do not sticky-merge stale cache.
  if (planIdentityChanged || amountChanged) {
