...
/**
 * Modal reason for TransferredAwayModal.
 * Admin revocation is silent (no expiry popup). Natural expiry still shows modal.
 *
 * @param {unknown} verifyResult
 * @returns {'suspended'|'expired'|null}
 */
export function resolveSubscriptionLossModalReason(verifyResult) {
  if (!isConfirmedSubscriptionLoss(verifyResult)) return null;
  if (isExplicitRevokedConfirmation(verifyResult)) return null;
