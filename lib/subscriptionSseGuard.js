...
function pickRetryableFlag(verifyResult) {
  ...
  return raw.retryable === true || data?.retryable === true;
}

function pickVerificationFailureToken(verifyResult) {
  const parts = [
    verifyResult?.error,
    verifyResult?.inactiveReason,
    verifyResult?.reason,
    verifyResult?.raw?.reason,
    verifyResult?.raw?.inactive_reason,
    verifyResult?.raw?.inactiveReason,
  ];
  return parts
...
/** Temporary / ambiguous verify — must not terminate an active playback session. */
export function isSubscriptionVerificationUnavailable(verifyResult) {
  ...
  const tokenBlob = pickVerificationFailureToken(verifyResult);
  if (
    tokenBlob.includes('pool_saturated') ||
    tokenBlob.includes('pool_acquire_timeout') ||
    tokenBlob.includes('query_timeout') ||
    tokenBlob.includes('connection reset') ||
    tokenBlob.includes('connection failure')
  ) {
    return true;
  }
  return false;
...
/**
 * Authoritative backend inactive — must not preserve AsyncStorage/cache entitlement.
 * Same predicate as {@link isConfirmedSubscriptionLoss}; explicit alias for reverify guards.
 */
export function isAuthoritativeInactiveEntitlement(verifyResult) {
...
/**
 * Former TransferredAwayModal reason helper.
 * Always null — expired-package / loss popups must never render.
 *
 * @param {unknown} verifyResult
 * @returns {null}
 */
...
export async function devicesShareIdentity(idA, idB) {
  const a = normalizeDeviceId(idA);
  const b = normalizeDeviceId(idB);
  if (!a || !b) return false;
  if (a === b) return true;
  const ids = await currentDeviceIdSet();
...
/**
 * Backend SSE reasons that mean subscription moved away — excludes recovery/migration/admin.
 * @param {unknown} sseReason
 */
export function isExplicitTransferRevokeReason(sseReason) {
...
/**
 * User-initiated transfer (Hamisha flow) vs background recovery/admin move.
 * @param {unknown} sseReason
 */
export function isUserConfirmedTransferReason(sseReason) {
  const r = String(sseReason ?? '').trim().toLowerCase();
 ...
 */
export function pickTransferSseReason(payload, eventName = '') {
  const inner = unwrapSubscriptionSsePayload(payload);
  if (!inner || typeof inner !== 'object') return '';
  const raw =
    inner.reason ??
    inner.transfer_reason ??
    inner.transferReason ??
    (eventName.includes('transfer') ? eventName : '');
  return raw != null ? String(raw).trim() : '';
}

/**
 * Runtime/OTA metadata for popup-request diagnostics. Lazy so node scripts
 * that import this module never touch expo-updates.
 *
 * @returns {Record<string, unknown>}
 */
function collectRuntimeUpdateMeta() {
  try {
    ...
    return {
      ...
      updateCreatedAt: Updates.createdAt ? String(Updates.createdAt) : null,
    };
  } catch (e) {
    return { runtimeMetaError: String(e?.message ?? e) };
  }
}

/**
 * Structured diagnostic for subscription lifecycle modal decisions.
 * Captures the full caller stack, API response, subscription state, route,
 * and OTA/runtime identity for every popup-decision request.
 *
 * @param {string} trigger
 * @param {unknown} verifyResult
 * @param {'revoked'|'suspended'|'expired'|null|'cleared'|'skipped'|string} decision
 * @param {Record<string, unknown>} [extra]
 */
export function logSubscriptionLossModalDecision(trigger, verifyResult, decision, extra = {}) {
  let apiResponse = null;
  try {
    apiResponse = JSON.stringify(verifyResult)?.slice(0, 2000) ?? null;
  } catch {
    apiResponse = '[unserializable]';
  }
  console.log('[SUBSCRIPTION_LOSS_MODAL]', trigger, {
    ...
    apiResponse,
    ...collectRuntimeUpdateMeta(),
    callerStack: new Error('SUBSCRIPTION_LOSS_MODAL_TRACE').stack ?? null,
    ...extra,
  });
}
