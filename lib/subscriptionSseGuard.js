...
/**
 * Extract inactive reason from a normalized verify/recover result.
...
export function isExplicitRevokedInactiveReason(reason) {
  ...
  if (!t) return false;
  if (t === 'transferred' || t.includes('transfer')) return false;
  return (
    t === 'revoked' ||
    t === 'suspended' ||
    t.includes('revoked') ||
...
/**
 * Modal reason for TransferredAwayModal — never "revoked" unless backend
 * verify response explicitly reports suspension/revocation (not SSE alone).
 *
 * @param {unknown} verifyResult
...
export function resolveSubscriptionLossModalReason(verifyResult) {
  ...
  const fromVerify = extractSubscriptionInactiveReason(verifyResult);
  if (isExplicitRevokedInactiveReason(fromVerify)) return 'revoked';
  if (isExplicitExpiredInactiveReason(fromVerify)) return 'expired';
  return 'expired';
}
