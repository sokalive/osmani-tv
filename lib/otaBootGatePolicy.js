...
/**
 * Bundles before the expired-package gate removal lack this marker.
 *
 * @returns {boolean}
 */
...
/**
 * Bundles before the final Home expired-package popup removal lack V2.
 *
 * @returns {boolean}
 */
export function hasKifurushiKimekwishaPopupRemovedV2() {
  try {
    // eslint-disable-next-line global-require
    const mod = require('./playbackGateCapability');
    return mod?.KIFURUSHI_KIMEKWISHA_POPUP_REMOVED_V2 === true;
  } catch {
    return false;
  }
}
...
/**
 * Bundles before the OTA auto-reload fix (reloadIfNew without embedded-only gate).
 *
 * @returns {boolean}
 */
export function hasKifurushiKimekwishaPopupRemovedV3() {
  try {
    // eslint-disable-next-line global-require
    const mod = require('./playbackGateCapability');
    return mod?.KIFURUSHI_KIMEKWISHA_POPUP_REMOVED_V3 === true;
  } catch {
    return false;
  }
}
...
/**
 * Bundles before popup-request stack-trace instrumentation lack V4.
 *
 * @returns {boolean}
 */
export function hasKifurushiKimekwishaPopupRemovedV4() {
  try {
    // eslint-disable-next-line global-require
    const mod = require('./playbackGateCapability');
    return mod?.KIFURUSHI_KIMEKWISHA_POPUP_REMOVED_V4 === true;
  } catch {
    return false;
  }
}
...
export function isStalePlaybackBundle() {
  ...
  if (!hasKifurushiKimekwishaPopupRemovedV2()) return true;
  if (!hasKifurushiKimekwishaPopupRemovedV3()) return true;
  if (!hasKifurushiKimekwishaPopupRemovedV4()) return true;
 ...
 * OR when explicitly requested via reloadIfNew (caller decides).
 *
...
export function shouldReloadAfterOtaFetch(staleAtSessionStart = isStalePlaybackBundle()) {
  ...
  if (isEmbeddedLaunchFlag()) return true;
  // Always allow reload when popup-removal markers are missing — do not depend
  // solely on Updates.isEmbeddedLaunch (unreliable on many VPS devices).
  if (!hasKifurushiKimekwishaPopupRemovedV3()) return true;
  if (!hasKifurushiKimekwishaPopupRemovedV4()) return true;
  return false;
}
...
export function collectOtaBootGateSnapshot() {
  ...
  return {
    ...
    kifurushiKimekwishaPopupRemovedV2: hasKifurushiKimekwishaPopupRemovedV2(),
    kifurushiKimekwishaPopupRemovedV3: hasKifurushiKimekwishaPopupRemovedV3(),
    kifurushiKimekwishaPopupRemovedV4: hasKifurushiKimekwishaPopupRemovedV4(),
