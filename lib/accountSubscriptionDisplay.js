...
/**
 * Display-only expiry for Account Box 4.
 *
 * Backend `expires_at` is never rewritten. When a legacy stacked entitlement
 * expires far beyond the assigned Admin plan window, present the date that
 * matches the capped remaining-days countdown so all four boxes agree with
 * the Admin plan. Playback entitlement is unchanged.
 *
 * @param {Record<string, unknown>|null|undefined} details
 * @param {string|null|undefined} subscriptionExpiresAt
 * @param {unknown[]} [catalogPlans]
 * @param {number|null} [nowMsOverride]
 * @returns {string|null}
 */
export function resolveAccountDisplayExpiresAt(
  ...
  const canonical = resolveCanonicalExpiresAt(details, subscriptionExpiresAt);
  if (!canonical) return null;

  const assignedDurationDays = resolveAssignedPlanDurationDays(details, catalogPlans);
  const remainingDays = resolveAccountRemainingDays(
    details,
    subscriptionExpiresAt,
    catalogPlans,
    nowMsOverride,
  );
  if (assignedDurationDays == null || remainingDays == null || remainingDays <= 0) {
    return canonical;
  }

  const canonicalMs = Date.parse(String(canonical));
  const nowMs =
    typeof nowMsOverride === 'number' && Number.isFinite(nowMsOverride)
      ? nowMsOverride
      : Date.now();
  if (!Number.isFinite(canonicalMs)) return canonical;

  // Backend expiry already within the assigned plan window from now — show truth.
  const planWindowMs = assignedDurationDays * 24 * 60 * 60 * 1000;
  if (canonicalMs <= nowMs + planWindowMs + 24 * 60 * 60 * 1000) {
    return canonical;
  }

  // Legacy stacked far-future expiry: present end matching capped remaining days.
  // Align to Africa/Dar_es_Salaam midnight of that calendar day (display only).
  ...
  // Midnight EAT of (today + remainingDays) as UTC instant.
  const displayUtcMs =
    Date.UTC(y, m, d + remainingDays, 0, 0, 0, 0) - EAT_OFFSET_MS;
  return new Date(displayUtcMs).toISOString();
}
