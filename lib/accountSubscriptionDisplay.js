...
/**
 * Legitimate package duration for this entitlement (display only).
 *
 * Prefer historical entitlement `planDurationDays` from the backend so a later
 * Admin catalog change cannot rewrite an older purchase. Catalog fills only
 * when the entitlement itself has no duration. Never use startedAt→expiresAt span.
 */
export function resolveAssignedPlanDurationDays(details, catalogPlans = []) {
  ...
  if (entitlementDays != null && entitlementDays > 0) {
    return Math.trunc(entitlementDays);
  }
...
/**
 * Display-only expiry for Account Box 4 ("Kuisha Tarehe").
 *
 * Always the authoritative backend `expires_at`. Never invent today+duration,
 * never stack, and never rewrite expiry on the client. Remaining/duration cards
 * separately cap presentation to the legitimate package duration.
 *
 * @param {Record<string, unknown>|null|undefined} details
...
export function resolveAccountDisplayExpiresAt(
  ...
  if (
    ...
    assignedDurationDays > 0 &&
    Number.isFinite(canonicalMs) &&
    canonicalMs > nowMs + assignedDurationDays * 24 * 60 * 60 * 1000 + 48 * 60 * 60 * 1000
  ) {
    traceAccountDisplay('expires_beyond_package_duration', {
      expiresAt: canonical,
      assignedDurationDays,
      remainingDays: resolveAccountRemainingDays(
        details,
