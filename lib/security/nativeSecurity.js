 ...
 * @param {string | null | undefined} expectedPackageName
 * @returns {Promise<import('../../modules/osmani-security').SecurityAuditResult | null>}
 */
export async function runNativeSecurityAudit(expectedCertSha256, expectedPackageName) {
  const mod = loadNative();
  if (!mod?.runSecurityAudit) return null;
  try {
    const raw = await mod.runSecurityAudit(
