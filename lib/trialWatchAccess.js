...
import { trialSettingsSnapshot, trialTrace } from './trialTrace';
 ...
 */
export async function getTrialChannelAccess(trialWatchSettings, meta = {}) {
  const settingsSnap = trialSettingsSnapshot(trialWatchSettings);
  const shouldApply = shouldApplyTrialWatch({
    isSubscribed: false,
    freeMode: false,
    trialWatchSettings,
  });
  if (!shouldApply) {
    trialTrace('trial_eligibility_denied', {
      reason: 'shouldApplyTrialWatch_false',
  ...
  trialTrace('trial_eligibility_result', {
    allowViaTrial: allow,
    phase: allowance.phase,
    remainingMs: allowance.remainingMs,
    trialConsumedMs: state.trialConsumedMs,
    trialExhausted: state.trialExhausted,
    settings: settingsSnap,
    ...meta,
  });
  return {
