 ...
 *
 * - direct_hls → native only (never WebView; bare HLS / direct media)
 ...
 *
 * @param {unknown} url
 * @param {'exo' | 'webview' | 'vlc' | 'native' | 'ijk' | 'direct_hls'} playerTypeNorm
...
export function playerTypeUsesNativeEngine(playerTypeNorm) {
  return (
    playerTypeNorm === 'exo' ||
    playerTypeNorm === 'vlc' ||
    playerTypeNorm === 'native' ||
    playerTypeNorm === 'ijk' ||
    playerTypeNorm === 'direct_hls'
  );
}
