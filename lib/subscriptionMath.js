 ...
 *   remainingMs: number,
 *   remainingDays: number,    // floor — matches Account stat card
...
export function computeSubscriptionProgress(input = {}) {
  ...
  const remainingMs = Math.max(0, rawRemainingMs ?? expiresMs - nowMs);
  const percentRemaining = Math.max(0, Math.min(100, (remainingMs / totalDurationMs) * 100));
  const percentUsed = 100 - percentRemaining;
  const remainingDays = Math.max(0, Math.floor(remainingMs / DAY_MS));
