import {
  LEGACY_HTTPS_API_FALLBACK,
  RENDER_PRODUCTION_API_URL,
  VPS_PRODUCTION_API_URL,
  resolveApiBaseUrl,
} from './apiBaseUrl';
...
export function getAdminApiBases() {
  ...
  if (primaryLower === VPS_PRODUCTION_API_URL.toLowerCase()) {
    bases.push(RENDER_PRODUCTION_API_URL);
  } else if (primary.startsWith('http://') && LEGACY_HTTPS_API_FALLBACK.startsWith('https://')) {
