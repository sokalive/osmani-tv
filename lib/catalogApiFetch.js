...
/** Default ceiling for admin API fetches — prevents multi-minute hangs on slow VPS links. */
export const ADMIN_API_TIMEOUT_MS = 12_000;
/** Tighter limit for subscription verify/recover (hot playback/payment paths). */
export const SUBSCRIPTION_API_TIMEOUT_MS = 10_000;
...
async function fetchAdminApiOnce(base, path, opts = {}) {
  ...
  if (opts.cacheBust === true) {
    ...
    requestPath = `${requestPath}${sep}_=${Date.now()}`;
  }
  ...
  console.log(
    `[${tag}]`,
    JSON.stringify({ phase: 'request', url, base, mode, timeoutMs: noTimeout ? 'none' : timeoutMs }),
  );

  const controller = typeof AbortController !== 'undefined' ? new AbortController() : null;
  let timeoutId = null;
  if (controller && !noTimeout && timeoutMs > 0) {
  ...
  try {
    res = await fetch(url, {
      ...
      headers: {
        ...
        'Cache-Control': opts.cacheBust ? 'no-cache' : 'no-cache',
