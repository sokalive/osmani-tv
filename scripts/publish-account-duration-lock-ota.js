...
/**
 * Publish Account UI duration-lock OTA (runtime 1.8.2 / versionCode 24).
 * Channels: preview, vps-preview, production — no new APK/AAB.
 *
 * Usage: node scripts/publish-account-duration-lock-ota.js
 */
...
const logPath = path.join(__dirname, '..', 'ota-publish-account-duration-lock.log');
const summaryPath = path.join(__dirname, '..', 'ota-publish-account-duration-lock-summary.json');
...
fs.writeFileSync(logPath, `[publish-account-duration-lock-ota] started ${new Date().toISOString()}\n`);

const msg =
  'fix(account): lock Muda wa Kifurushi/remaining to entitlement planDurationDays; Kuisha Tarehe=backend expires_at runtime 1.8.2';
