#!/usr/bin/env node
'use strict';

/**
 * Publish sticky-playback / deferred-expiry / mid-session resolve OTA.
 * Targets production (user) + preview (emulator verification channel).
 * Runtime 1.8.2 / Version 24 only — no new APK/AAB.
 *
 * Usage: node scripts/publish-sticky-playback-ota.js
 */

const { spawnSync, execSync } = require('child_process');
const fs = require('fs');
const path = require('path');

const RUNTIME = '1.8.2';
const CHANNELS = ['production', 'preview'];
const NPX = process.platform === 'win32' ? 'npx.cmd' : 'npx';
const logPath = path.join(__dirname, '..', 'ota-publish-sticky-playback.log');
const summaryPath = path.join(__dirname, '..', 'ota-publish-sticky-playback-summary.json');

function appendLog(text) {
  try {
    fs.appendFileSync(logPath, text + '\n');
  } catch {}
}

console.log('=== OTA SAFETY CHECK ===');
console.log(`Channels: ${CHANNELS.join(', ')}`);
console.log(`Runtime: ${RUNTIME}`);
console.log('versionCode: unchanged (24)');
console.log('APK/AAB: NONE');
console.log('========================\n');

fs.writeFileSync(logPath, `[publish-sticky-playback-ota] started ${new Date().toISOString()}\n`);

const msg =
  'fix(playback): sticky session auth + deferred wall-clock expiry + mid-session resolveSource runtime 1.8.2';
const quotedMsg = msg.replace(/"/g, '');

const results = [];

for (const channel of CHANNELS) {
  const envFlag = channel === 'production' ? '--environment production' : '';
  const cmd =
    `${NPX} eas-cli update --channel ${channel} ${envFlag} `.replace(/\s+/g, ' ').trim() +
    ` --message "${quotedMsg}" --non-interactive`;

  console.log(`\nPublishing ${channel}: ${cmd}\n`);
  appendLog(`TARGET channel=${channel} runtime=${RUNTIME}`);

  const result = spawnSync(cmd, {
    stdio: 'pipe',
    shell: true,
    encoding: 'utf8',
    env: {
      ...process.env,
      CI: '1',
      EAS_SKIP_AUTO_FINGERPRINT: '1',
      OTA_RUNTIME_TARGET: RUNTIME,
      EXPO_PUBLIC_API_URL: 'https://api.osmanitv.com',
    },
  });

  const out = `${result.stdout || ''}${result.stderr || ''}`;
  process.stdout.write(out);
  appendLog(out);

  if (result.status !== 0) {
    console.error(`FAILED channel=${channel} exit ${result.status}`);
    appendLog(`FAILED channel=${channel} exit ${result.status}`);
    process.exit(result.status ?? 1);
  }

  const groupMatch =
    out.match(/Update group ID\s+([a-f0-9-]{36})/i) ||
    out.match(/group[=:\s]+([a-f0-9-]{36})/i);
  const androidMatch = out.match(/Android update ID\s+([a-f0-9-]{36})/i);

  results.push({
    channel,
    runtime: RUNTIME,
    groupId: groupMatch ? groupMatch[1] : null,
    androidUpdateId: androidMatch ? androidMatch[1] : null,
  });
}

const summary = {
  timestamp: new Date().toISOString(),
  commit: execSync('git rev-parse HEAD', { encoding: 'utf8' }).trim(),
  runtime: RUNTIME,
  versionCode: 24,
  versionName: '1.8.2',
  newApkCreated: false,
  newAabCreated: false,
  channels: results,
};

fs.writeFileSync(summaryPath, JSON.stringify(summary, null, 2));
console.log('\n[publish-sticky-playback-ota] published');
console.log(JSON.stringify(summary, null, 2));
