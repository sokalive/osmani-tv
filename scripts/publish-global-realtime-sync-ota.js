...
/**
 * Publish global admin realtime catalog sync OTA to every Play runtime.
 * Usage: node scripts/publish-global-realtime-sync-ota.js
 */
...
const logPath = path.join(__dirname, '..', 'ota-publish-global-realtime-sync.log');
const groups = [];
...
fs.writeFileSync(logPath, `[publish-global-realtime-sync-ota] started ${new Date().toISOString()}\n`);

for (const runtime of RUNTIMES) {
  const msg = `fix(realtime): global admin catalog SSE sync runtime ${runtime}`;
  console.log(`\n=== OTA runtime ${runtime} ===`);
  appendLog(`\n=== OTA runtime ${runtime} ===`);
...
const summaryPath = path.join(__dirname, '..', 'ota-publish-global-realtime-sync-summary.json');
fs.writeFileSync(summaryPath, JSON.stringify(summary, null, 2));

console.log('\n[publish-global-realtime-sync-ota] all runtimes published');
console.log('[publish-global-realtime-sync-ota] summary', JSON.stringify(groups, null, 2));
