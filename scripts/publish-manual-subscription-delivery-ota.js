...
/**
 * Publish manual subscription instant delivery fix OTA to every Play runtime (16–24).
 * Usage: node scripts/publish-manual-subscription-delivery-ota.js
 */
...
const logPath = path.join(__dirname, '..', 'ota-publish-manual-subscription-delivery.log');
const groups = [];
...
fs.writeFileSync(logPath, `[publish-manual-subscription-delivery-ota] started ${new Date().toISOString()}\n`);

for (const runtime of RUNTIMES) {
  const msg = `fix(subscription): instant manual grant device stream + SSE aliases runtime ${runtime}`;
  console.log(`\n=== OTA runtime ${runtime} ===`);
  appendLog(`\n=== OTA runtime ${runtime} ===`);
...
const summaryPath = path.join(__dirname, '..', 'ota-publish-manual-subscription-delivery-summary.json');
