...
/**
 * Publish MFALME manual subscription gift restoration OTA to every Play runtime (16–24).
 *
 * Usage: node scripts/publish-manual-subscription-ota.js
 */
...
const logPath = path.join(__dirname, '..', 'ota-publish-manual-subscription.log');
const groups = [];
...
for (const runtime of RUNTIMES) {
  const msg = `fix(subscription): restore MFALME manual gift ack key runtime ${runtime}`;
  console.log(`\n=== OTA runtime ${runtime} ===`);
  appendLog(`\n=== OTA runtime ${runtime} ===`);
...
const summaryPath = path.join(__dirname, '..', 'ota-publish-manual-subscription-summary.json');
fs.writeFileSync(summaryPath, JSON.stringify(summary, null, 2));

console.log('\n[publish-manual-subscription-ota] all runtimes published');
console.log('[publish-manual-subscription-ota] summary', JSON.stringify(groups, null, 2));
console.log('[publish-manual-subscription-ota] wrote', summaryPath);
