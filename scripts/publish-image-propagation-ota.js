...
/**
 * Publish image propagation fix OTA to every Play runtime (16–24).
 * Usage: node scripts/publish-image-propagation-ota.js
 */
...
const logPath = path.join(__dirname, '..', 'ota-publish-image-propagation.log');
const groups = [];
...
fs.writeFileSync(logPath, `[publish-image-propagation-ota] started ${new Date().toISOString()}\n`);

for (const runtime of RUNTIMES) {
  const msg = `fix(images): VPS upload propagation + CDN fallback runtime ${runtime}`;
  console.log(`\n=== OTA runtime ${runtime} ===`);
  appendLog(`\n=== OTA runtime ${runtime} ===`);
...
const summaryPath = path.join(__dirname, '..', 'ota-publish-image-propagation-summary.json');
