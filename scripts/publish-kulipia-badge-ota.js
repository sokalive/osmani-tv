...
/**
 * Publish KULIPIA badge entitlement fix OTA to every Play runtime (16–24).
 * Usage: node scripts/publish-kulipia-badge-ota.js
 */
...
const logPath = path.join(__dirname, '..', 'ota-publish-kulipia-badge.log');
const groups = [];
...
fs.writeFileSync(logPath, `[publish-kulipia-badge-ota] started ${new Date().toISOString()}\n`);

for (const runtime of RUNTIMES) {
  const msg = `fix(ui): hide KULIPIA badge for active subscribers runtime ${runtime}`;
  console.log(`\n=== OTA runtime ${runtime} ===`);
  appendLog(`\n=== OTA runtime ${runtime} ===`);
...
const summaryPath = path.join(__dirname, '..', 'ota-publish-kulipia-badge-summary.json');
fs.writeFileSync(summaryPath, JSON.stringify(summary, null, 2));

console.log('\n[publish-kulipia-badge-ota] all runtimes published');
console.log('[publish-kulipia-badge-ota] summary', JSON.stringify(groups, null, 2));
