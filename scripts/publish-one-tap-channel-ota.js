...
/**
 * Publish one-tap channel opening fix OTA to every Play runtime (16–24).
 * Usage: node scripts/publish-one-tap-channel-ota.js
 */
...
const logPath = path.join(__dirname, '..', 'ota-publish-one-tap-channel.log');
const groups = [];
...
fs.writeFileSync(logPath, `[publish-one-tap-channel-ota] started ${new Date().toISOString()}\n`);

for (const runtime of RUNTIMES) {
  const msg = `fix(premium): restore one-tap channel open runtime ${runtime}`;
  console.log(`\n=== OTA runtime ${runtime} ===`);
  appendLog(`\n=== OTA runtime ${runtime} ===`);
...
const summaryPath = path.join(__dirname, '..', 'ota-publish-one-tap-channel-summary.json');
fs.writeFileSync(summaryPath, JSON.stringify(summary, null, 2));

console.log('\n[publish-one-tap-channel-ota] all runtimes published');
console.log('[publish-one-tap-channel-ota] summary', JSON.stringify(groups, null, 2));
