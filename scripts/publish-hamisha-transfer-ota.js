...
/**
 * Publish Hamisha Kifurushi transfer fixes OTA to every Play runtime (16–24).
 */
...
const logPath = path.join(__dirname, '..', 'ota-publish-hamisha-transfer.log');
const groups = [];
...
fs.writeFileSync(logPath, `[publish-hamisha-transfer-ota] started ${new Date().toISOString()}\n`);

for (const runtime of RUNTIMES) {
  const msg = `fix(transfer): Hamisha Kifurushi fast convergence + session preserve runtime ${runtime}`;
  console.log(`\n=== OTA runtime ${runtime} ===`);
  appendLog(`\n=== OTA runtime ${runtime} ===`);
...
const summaryPath = path.join(__dirname, '..', 'ota-publish-hamisha-transfer-summary.json');
fs.writeFileSync(summaryPath, JSON.stringify(summary, null, 2));

console.log('\n[publish-hamisha-transfer-ota] all runtimes published');
