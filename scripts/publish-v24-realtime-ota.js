...
/**
 * Publish v24 VPS-first realtime state OTA to every Play runtime (16–24).
 * Usage: node scripts/publish-v24-realtime-ota.js
 */
...
const logPath = path.join(__dirname, '..', 'ota-publish-v24-realtime.log');
const groups = [];
...
fs.writeFileSync(logPath, `[publish-v24-realtime-ota] started ${new Date().toISOString()}\n`);

for (const runtime of RUNTIMES) {
  const msg = `fix(realtime): v24 zero-stale revoke/grant/channel state runtime ${runtime}`;
  console.log(`\n=== OTA runtime ${runtime} ===`);
  appendLog(`\n=== OTA runtime ${runtime} ===`);
...
const summaryPath = path.join(__dirname, '..', 'ota-publish-v24-realtime-summary.json');
fs.writeFileSync(summaryPath, JSON.stringify(summary, null, 2));

console.log('\n[publish-v24-realtime-ota] all runtimes published');
console.log('[publish-v24-realtime-ota] summary', JSON.stringify(groups, null, 2));
