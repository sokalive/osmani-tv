...
import { getBanners, getChannels, getServerHealth, invalidateBannersCatalogCache, invalidateCatalogCache } from '../api';
...
import { ADMIN_BANNER_SSE_EVENTS, ADMIN_RUNTIME_MODE_SSE_EVENTS, ADMIN_SOFT_REFRESH_SSE_EVENTS } from '../lib/adminSseRefreshEvents';
import { notifyAdminSettingsRefresh } from '../lib/adminSettingsRefresh';
import {
...
/** SSE names that carry free / emergency / maintenance ΓÇö must not share the catalog debouncer. */
...
/** Admin flags poll while foreground; SSE is primary ΓÇö conservative interval for cost. */
...
const MfalmeAppContext = createContext(null);
...
function shouldApplyTrialWatchServerPatch(currentVersion, nextSettings) {
  if (!nextSettings?.configLoaded) return false;
  const nextVersion = nextSettings.version;
  if (nextVersion == null) return true;
  if (currentVersion == null) return true;
  return nextVersion >= currentVersion;
}

export function MfalmeAppProvider({ children }) {
  ...
  const trialWatchVersionRef = useRef(null);
  ...
  /**
   * Pre-playback gate. EVERY playback attempt MUST call this ΓÇö the
   ...
   * @param {{ showGlobalLoading?: boolean }} [opts] ΓÇö set showGlobalLoading: false for pull-to-refresh (no full-screen blocking load).
   */
  /**
  ...
  const refreshTrialWatchSettings = useCallback(async (reason = 'poll') => {
    try {
      ...
      if (s && shouldApplyTrialWatchServerPatch(trialWatchVersionRef.current, s)) {
        trialWatchSettingsRef.current = s;
        trialWatchVersionRef.current = s.version ?? trialWatchVersionRef.current;
        setTrialWatchSettings(s);
        devLog('[TRIAL_WATCH_SYNC]', reason, s);
      } else if (!s) {
        trialWatchSettingsRef.current = TRIAL_WATCH_FAIL_CLOSED;
        trialWatchVersionRef.current = null;
        setTrialWatchSettings(TRIAL_WATCH_FAIL_CLOSED);
        devLog('[TRIAL_WATCH_SYNC]', reason, 'fail_closed_no_runtime_config');
      } else {
        devLog('[TRIAL_WATCH_SYNC]', reason, 'skip_stale_version', {
          current: trialWatchVersionRef.current,
          incoming: s.version,
        });
      }
    } finally {
  ...
  const refreshSettingsOnly = useCallback(async (reason = 'poll') => {
    ...
    void refreshServerHealth(reason);
    notifyAdminSettingsRefresh(reason);
  }, [refreshServerHealth, refreshTrialWatchSettings]);

  const refreshBannersOnly = useCallback(async (reason = 'banners') => {
    invalidateBannersCatalogCache();
    try {
      const nextBanners = await getBanners({ force: true });
      if (!Array.isArray(nextBanners)) return;
      const enriched = enrichBannersForViewer(nextBanners);
      setRawBanners(enriched);
      await dropLegacyBannersCache();
      await writeBannersCache(nextBanners);
      logBannerRuntimeDiagnostics(enriched);
      devLog('[BANNERS_SYNC]', reason, enriched.length);
    } catch (e) {
      devLog('[BANNERS_SYNC]', reason, 'fetch_failed', e?.message ?? e);
    }
  }, []);
  ...
  const refresh = useCallback(async (opts = {}) => {
    ...
    try {
      ...
      if (trialFlags && shouldApplyTrialWatchServerPatch(trialWatchVersionRef.current, trialFlags)) {
        trialWatchSettingsRef.current = trialFlags;
        trialWatchVersionRef.current = trialFlags.version ?? trialWatchVersionRef.current;
        setTrialWatchSettings(trialFlags);
      } else if (!trialFlags && !skipSettingsFromHttp) {
        trialWatchSettingsRef.current = TRIAL_WATCH_FAIL_CLOSED;
        trialWatchVersionRef.current = null;
        setTrialWatchSettings(TRIAL_WATCH_FAIL_CLOSED);
      }
  ...
  const scheduleAdminDrivenSoftSync = useCallback(
    (reason = 'sse:admin') => {
      ...
      adminSoftSyncTimerRef.current = setTimeout(() => {
        ...
        void refreshSettingsOnly(reason);
    ...
    [refresh, refreshSettingsOnly, refreshTrialWatchSettings, reverifySubscription],
  );
  ...
  // Cold-start: recover (in case of reinstall) and verify ΓÇö background, never blocks splash.
  useEffect(() => {
    (async () => {
      try {
        await withTimeout(
  ...
  useEffect(() => {
    ...
    // `transfer_completed` for context purposes ΓÇö the destination
    ...
    const offRejected = subscribeRealtimeEvent('transfer_rejected', (payload) => {
      if (!sourceTransferSessionRef.current?.code) return;
      console.log('[TRANSFER_REJECTED]', 'sse', payload);
      sourceTransferSessionRef.current = null;
      setPendingTransfer(null);
    });
    // Source-device approve/reject popup fires on EITHER event name ΓÇö
    ...
    const offRuntimeModes = RUNTIME_MODE_SSE_NAMES.map((ev) =>
      subscribeRealtimeEvent(ev, (payload) => {
        ...
        setTrialWatchSettings((prev) => {
          ...
          if (!shouldApplyTrialWatchServerPatch(trialWatchVersionRef.current, parsed)) {
            return prev;
          }
          trialWatchSettingsRef.current = parsed;
          trialWatchVersionRef.current = parsed.version ?? trialWatchVersionRef.current;
          return parsed;
        });
    ...
    const offBannerEvents = ADMIN_BANNER_SSE_EVENTS.map((ev) =>
      subscribeRealtimeEvent(ev, (payload) => {
        console.log('[BANNERS_SSE]', ev, payload);
        void refreshBannersOnly(`sse:${ev}`);
      }),
    );
    return () => {
      offRevoked();
      ...
      offBannerEvents.forEach((off) => off());
    };
  }, [refresh, refreshBannersOnly, refreshTrialWatchSettings, reverifySubscription, scheduleAdminDrivenSoftSync]);
  ...
  useEffect(() => {
    ...
    const sub = AppState.addEventListener('change', (next) => {
      ...
      if (next === 'active') {
        schedule(1000);
        void refreshBannersOnly('app-resume');
  ...
  }, [refresh, refreshBannersOnly, reverifySubscription]);
  ...
  const triggerPendingTransfer = useCallback((payload, reason = 'external') => {
    const session = sourceTransferSessionRef.current;
    if (!session?.code) {
      console.log('[transfer-ui]', 'ignored pending transfer ΓÇö no source session', { reason });
      return;
    }
    if (!payload || typeof payload !== 'object') {
    ...
    const sessionBare = bareTransferCode(session.code);
    if (eventBare && sessionBare && eventBare !== sessionBare) {
      console.log('[transfer-ui]', 'ignored pending transfer ΓÇö code mismatch', {
        reason,
        eventCode: code,
        sessionCode: session.code,
      });
  ...
  return <MfalmeAppContext.Provider value={value}>{children}</MfalmeAppContext.Provider>;
}

export function useMfalmeApp() {
  const ctx = useContext(MfalmeAppContext);
  if (!ctx) {
    throw new Error('useMfalmeApp must be used within MfalmeAppProvider');
  }
  return ctx;
}
