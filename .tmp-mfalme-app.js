...
import ToaKifurushiScreen from './screens/ToaKifurushiScreen';
import SecurityCenterScreen from './screens/SecurityCenterScreen';
import AdminSecurityScreen from './screens/AdminSecurityScreen';
import { MfalmeAppProvider, useMfalmeApp } from './context/MfalmeAppContext';
...
import { getAndroidBlurMethod, getExpoImageExpoGoAndroidProps } from './lib/expoGoAndroidRenderCompat';
import { dispatchMfalmeDeepLink } from './lib/mfalmeDeepLinkDispatch';
import MfalmeDeepLinkGate from './components/MfalmeDeepLinkGate';
...
import { openExternalWebsiteChannelFromSnapshot } from './lib/openExternalWebsiteChannelFromSnapshot';
import { isExternalWebsiteLaunch } from './lib/externalWebsiteChannel';
...
import { logApiConfigDiagnostics } from './lib/apiConfigDiagnostics';
...
/** Cold start: deep link may arrive before NavigationContainer is ready ΓÇö flush in onReady. */
const pendingMfalmeUrlRef = { current: /** @type {string | null} */ (null) };
...
const mfalmeLinking = {
  ...
  config: {
    screens: {
      ...
      /** Channel opens are handled in MfalmeDeepLinkGate (catalog + subscription gate). */
    },
  },
};
...
/** Home ΓÇ£Maarufu / VipengeleΓÇ¥ horizontal strips */
...
const COLORS = {
  /** Near-black with subtle deep red ΓÇö stadium / premium night pitch */
  background: '#0C0608',
  card: '#151014',
  live: '#1BCB5A',
  free: '#2AAE5E',
  ...
  nav: '#12090C',
  white: '#FFFFFF',
  /** Bottom tabs ΓÇö classic warm ΓÇ£HomeΓÇ¥ emphasis */
  tabActive: '#FFB347',
  tabInactive: '#DDE3EC',
};
...
function ChannelCatalogScreen({
  ...
  const handleCardPress = useCallback(
    async (item) => {
      ...
      const channelId = String(item?.playerChannel?.id ?? item?.id ?? '').trim();
      const found = channelId ? findRawChannelById(rawChannels, channelId) : null;
      if (found && isExternalWebsiteLaunch(found.raw)) {
        const snapshot = await awaitPremiumAccessSnapshot();
        await openExternalWebsiteChannelFromSnapshot(snapshot, {
          raw: found.raw,
          playerChannel: item.playerChannel,
          cardIsPremium: item.isPremium,
          openPaymentModal: openPremiumModal,
          verifySubscriptionBeforePlay,
          security,
          Alert,
        ...
        <View style={styles.cardImageWrap}>
          {item.thumbnailUri ? (
            <ExpoImage
              ...
              contentFit="cover"
              transition={120}
              {...getExpoImageExpoGoAndroidProps()}
            />
          ) : (
      ...
      <View>
        <View style={styles.topRow}>
          <View style={styles.titleWrap}>
            <View style={styles.titleLine}>
              <OtaDebugTitleTap>
                <Text style={styles.appTitle}>MFALME TV</Text>
              </OtaDebugTitleTap>
        ...
        {channelsPendingInitialLoad ? (
          <View style={styles.channelsStatusRow}>
            ...
            <Text style={styles.channelsStatusText}>Inapakia chaneliΓÇª</Text>
          </View>
        ) : null}
        {isOffline ? (
...
function GlobalEmergencyGate() {
  const { emergencyMode, emergencyModalRequestVersion } = useMfalmeApp();
...
function MfalmeLovableTabBar(props) {
  const insets = useSafeAreaInsets();
      ...
      {Platform.OS === 'web' ? (
        ...
        <>
          <BlurView
            ...
            tint="dark"
            experimentalBlurMethod={getAndroidBlurMethod()}
...
function RootNavigator() {
  return (
    <Stack.Navigator screenOptions={{ headerShown: false }}>
      ...
      />
      <Stack.Screen name="ToaKifurushi" component={ToaKifurushiScreen} />
...
function AppTabs() {
  return (
    <Tab.Navigator
      tabBar={(props) => <MfalmeLovableTabBar {...props} />}
      screenOptions={({ route }) => ({
        headerShown: false,
...
function AppShell({ navigationRevision, setNavigationRevision }) {
  ...
  useEffect(() => {
    logApiConfigDiagnostics('app_boot_ready');
    ...
    const stopOneSignal = setupOneSignal({
      onOpenUrl: dispatchMfalmeDeepLink,
    });
    return () => {
      void stopPresence();
      stopRealtimeSync();
          ...
          <SecurityProvider>
            ...
            <NavigationContainer
              ...
              onReady={() => {
                ...
                const pending = pendingMfalmeUrlRef.current;
                if (pending) {
                  pendingMfalmeUrlRef.current = null;
                  dispatchMfalmeDeepLink(pending);
                }
              }}
            ...
            >
              ...
              <MfalmeDeepLinkGate
                ...
                pendingUrlRef={pendingMfalmeUrlRef}
              />
            </NavigationContainer>
            <GlobalEmergencyGate />
