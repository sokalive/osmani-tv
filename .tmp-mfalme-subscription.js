...
function subscriptionApiRoot() {
  return `${getApiBaseUrl().replace(/\/+$/, '')}/api`;
}

export const SUB_CACHE_KEYS = Object.freeze({
  active: 'mfalme:sub:active',
  expiresAt: 'mfalme:sub:expires_at',
  deviceId: 'mfalme:sub:device_id',
  fingerprint: 'mfalme:sub:fingerprint',
  revokedAt: 'mfalme:sub:revoked_at',
});
 ...
 * Note: `active` here is the backend's verdict ΓÇö the app does NOT
...
export async function verifySubscription(deviceId, deviceFingerprint) {
  const url = `${subscriptionApiRoot()}/subscription/verify`;
...
/**
 * Reinstall recovery ΓÇö ask the backend if any active subscription is
...
export async function recoverSubscription(deviceId, deviceFingerprint) {
  const url = `${subscriptionApiRoot()}/subscription/recover`;
...
export async function redeemOfferCode(deviceId, deviceFingerprint, offerCode) {
  const url = `${subscriptionApiRoot()}/subscription/redeem-offer-code`;
...
export async function acknowledgeManualGift(deviceId, deviceFingerprint, manualGiftAckKey) {
  const url = `${subscriptionApiRoot()}/subscription/acknowledge-manual-gift`;
...
export async function initiateTransfer(deviceId, deviceFingerprint, phone = '') {
  const url = `${subscriptionApiRoot()}/transfer/request`;
 ...
 * subscription on this device ΓÇö we surface a `{ status: 'pending' }`
...
export async function redeemTransfer(code, deviceId, deviceFingerprint) {
  const url = `${subscriptionApiRoot()}/transfer/confirm`;
...
export async function respondToTransfer(code, decision) {
  const url = `${subscriptionApiRoot()}/transfer/respond`;
 ...
 * the backend adds (or renames) the route. Failure modes never throw ΓÇö the
...
export async function getTransferStatus(code) {
  ...
  const candidates = TRANSFER_STATUS_PATHS.flatMap((path) => {
    return [codeWithPrefix, codeBare].map((c) =>
      `${subscriptionApiRoot()}${path.replace('{code}', encodeURIComponent(c))}`,
    );
  });
...
/* -----------------------------------------------------------------
 * Local cache (UI hint only ΓÇö never used for trust decisions).
 * ----------------------------------------------------------------- */
