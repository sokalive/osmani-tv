/**
 * Viewer-facing banner JSON from GET /api/banners.
 *
 * Strips green (Swahili) and blue (clock) runtime timer UI by disabling countdown/timer
 * flags in the API payload. Preserves the admin red badge only and normalizes it for
 * top-left overlay rendering (badge_position / runtime_position hints).
 *
 * Deploy this on osmani-admin-api inside the /api/banners handler so older APK builds
 * receive the shape without an app update. The mobile app also applies the same transform
 * in api.js getBanners() as a safety net.
 */
 ...
 * @returns {unknown}
 */
function enrichBannersForViewer(value) {
...
module.exports = {
  RED_BADGE,
  enrichBannerForViewer,
  enrichBannersForViewer,
};
